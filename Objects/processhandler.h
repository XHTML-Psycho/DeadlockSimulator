...
#include <QThread>
#include <QDebug>
#include <Algorithms/BankersAlgorithm.h>
#include <Algorithms/directallocationsim.h>
#include <Algorithms/circularwaithandler.h>
#include <Algorithms/holdandwaithandler.h>
#include <Algorithms/strictlockpolicy.h>

//Q_DECLARE_METATYPE(QList<ManagedProcess>)

/**
 * @brief Class represents the process handler which is responsible for the whole simulation process
 */
...
public:
    /**
     * @brief creates a Process Handler with the given parameters and also initializes the semaphores
     * @param processes is the process belonging to the handler object
     ...
     */
    explicit ProcessHandler(ManagedProcess processes, int selectedAlgorithm, QObject *parent = 0);

    /**
     * @brief refreshProcess updates the neededResources List in the member Process
     * @param nextResource is the ID of the resource that has been reserved
     * @param countResource is the count of the resource
     */
    void refreshProcess(int nextResource, int countResource);

    /**
     * @brief configureAllocations used to copy the assignedResources_C and stillNeededResources_R from mainwindow to the threads
     * @param assignedResources_C matrix containing which and how many resources a process is occupying
     * @param stillNeededResources_R matrix containing which and how many resources a process will still need to occupy throughout the simulation
     */
    void configureAllocations(int assignedResources_C[3][4], int stillNeededResources_R[3][4]){
        for(int i = 0; i < 3; i++){
            for(int j = 0; j < 4; j++){
    ...
    /**
     * @brief configureAlgorithm sets the algorithm used for the simulation
     * @param algorithm number to choose the algorithm
     */
    void configureAlgorithm(int algorithm){
...
signals:
     ...
     * @param processID is the ID of the process member to determine the process in the main thread
...
public:
    ...
    static int stillNeededResources_R[3][4]; ///< matrix containing which and how many resources a process will still need to occupy throughout the simulation

private:
    ManagedProcess process; ///< process is the process running in the thread
